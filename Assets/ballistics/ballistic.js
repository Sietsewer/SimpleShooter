...
{
	time = Time.time;
	//spawnpoint = GameObject.Find("/lol/cam/gun/m9/spawnpoint");
...
function Update () 
{
	...
	transform.position += transform.forward * Vector3.Magnitude(v)*Time.deltaTime ; // travel speed
	transform.position -= transform.forward * kv*Vector3.Magnitude(v)*Time.deltaTime ; //air friction (just remove speed proportionally to the actual speed of the bullet)  
	...
	if (Physics.Linecast(previousPos, currentPos, hit, layerMask) && (Time.time-time) > 0.001) 
	{
		if (hit.rigidbody)
		{
			hit.rigidbody.AddForceAtPosition(kec*ec * transform.forward, hit.point);
		}
		
		Debug.DrawRay(hit.point,hit.normal*1,Color.green, dur);
		angleContact = Vector3.Angle(transform.forward,hit.normal);
